...
def launch_fe(data):
    ...
    columns_list = ["system_name", "june_precipitation", "july_precipitation", "august_precipitation", "pesticide_material_applied", "pesticide_material_quantity", "fertilizer_material_applied", "fertilizer_material_quantity", "planting_quantity", "Unnamed: 0", "tomato_mh_dry_yield_Mg/ha", "march_precipitation", "april_precipitation", "may_precipitation", "march_ETo", "april_ETo", "may_ETo", "june_ETo", "july_ETo", "august_ETo", "march_solrad", "april_solrad", "may_solrad", "june_solrad", "july_solrad", "august_solrad", "march_maxairtemp", "april_maxairtemp", "may_maxairtemp", "june_maxairtemp", "july_maxairtemp", "august_maxairtemp", "march_avgrelhum", "april_avgrelhum", "may_avgrelhum", "june_avgrelhum", "july_avgrelhum", "august_avgrelhum"]
    ...
    cat_model_impute = \
        SimpleImputer(strategy='most_frequent', fill_value='missing').fit(dataset[["june_precipitation", "august_precipitation"]])
    # Save the model
    model_name = "9658f1be-2fcb-430f-8f6e-cb1bae56f263"
    fh = open(model_name, "wb")
    pickle.dump(cat_model_impute, fh)
    fh.close()

    cat_features = ["june_precipitation", "august_precipitation"]
    dataset[cat_features] = \
        cat_model_impute.transform(dataset[cat_features])

    # Fill values missing in continuous features
    cont_model_impute = \
        SimpleImputer(strategy='median').fit(dataset[["april_precipitation", "may_precipitation", "april_solrad", "may_solrad", "june_solrad", "july_solrad", "august_solrad", "april_maxairtemp", "may_maxairtemp", "june_maxairtemp", "july_maxairtemp", "august_maxairtemp", "march_avgrelhum", "april_avgrelhum", "may_avgrelhum", "june_avgrelhum", "july_avgrelhum", "august_avgrelhum"]])
    # Save the model
    model_name = "6bef52bd-fa5e-42e3-8973-45045737af04"
    fh = open(model_name, "wb")
    pickle.dump(cont_model_impute, fh)
    fh.close()

    cont_features = ["april_precipitation", "may_precipitation", "april_solrad", "may_solrad", "june_solrad", "july_solrad", "august_solrad", "april_maxairtemp", "may_maxairtemp", "june_maxairtemp", "july_maxairtemp", "august_maxairtemp", "march_avgrelhum", "april_avgrelhum", "may_avgrelhum", "june_avgrelhum", "july_avgrelhum", "august_avgrelhum"]
    ...
    # Save the model
    model_name = "2123a7b7-57d9-427e-8dba-4ae848169828"
    fh = open(model_name, "wb")
...
def get_model_id():
    return "2123a7b7-57d9-427e-8dba-4ae848169828"
...
file1 = open("/content/drive/Shareddrives/1:1 Meenakshi Nair/CropYieldA_GBR.txt", "a")  # append mode
...
print (min_rmse)
print (min_rmse, min_md, min_lr, min_ne)
...
file1 = open("/content/drive/Shareddrives/1:1 Meenakshi Nair/CropYieldH_GBR.txt", "a")  # append mode
...
file1 = open("/content/drive/Shareddrives/1:1 Meenakshi Nair/CropYieldH_RNN.txt", "a")  # append mode
