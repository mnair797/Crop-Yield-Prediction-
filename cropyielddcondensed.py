def launch_fe(data):
    ...
    cont_model_impute = \
        SimpleImputer(strategy='median').fit(dataset[["may_minairtemp", "june_minairtemp", "july_minairtemp", "august_minairtemp", "june_avgrelhum", "august_avgrelhum", "june_dew", "august_dew"]])
    # Save the model
    model_name = "0a393668-660c-4c23-a294-3d870d26eb9e"
    fh = open(model_name, "wb")
    pickle.dump(cont_model_impute, fh)
    fh.close()

    cont_features = ["may_minairtemp", "june_minairtemp", "july_minairtemp", "august_minairtemp", "june_avgrelhum", "august_avgrelhum", "june_dew", "august_dew"]
    ...
    # Save the model
    model_name = "82dd8b85-4f62-44d9-aaee-298a9a592a80"
    fh = open(model_name, "wb")
...
file1 = open("/content/drive/Shareddrives/1:1 Meenakshi Nair/CropYieldD_GBR.txt", "a")  # append mode
...
gbr = GradientBoostingRegressor(n_estimators=380, max_depth=5, learning_rate=0.009, min_samples_split=3)
gbr.fit(train.iloc[:,1:], train.iloc[:,0])
...
rmse = np.sqrt(np.mean((y_pred - test.iloc[:, 0])**2))
# import the library to calculate mae
print (mean_absolute_error(np.array(test.iloc[:, 0]), y_pred))

print (np.mean(np.abs((np.array(test.iloc[:, 0]) - y_pred)/np.array(test.iloc[:, 0])))*100)

#RNN

print ("RNN!!")

file1 = open("/content/drive/Shareddrives/1:1 Meenakshi Nair/CropYieldD_RNN.txt", "a")  # append mode
...
for e in range (40,100,+10):
  for b_e in range (10,60,+4):
    rnn.fit(x_training_data, y_training_data, epochs = e, batch_size = b_e)
