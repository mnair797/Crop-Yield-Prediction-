...
def launch_fe(data):
    ...
    columns_list = ["june_precipitation", "july_precipitation", "august_precipitation", "pesticide_material_applied", "pesticide_material_quantity", "fertilizer_material_applied", "fertilizer_material_quantity", "planting_quantity", "tomato_mh_dry_yield_Mg/ha", "march_precipitation", "april_precipitation", "may_precipitation", "march_ETo", "april_ETo", "may_ETo", "june_ETo", "july_ETo", "august_ETo", "march_solrad", "april_solrad", "may_solrad", "june_solrad", "july_solrad", "august_solrad"]
    ...
    cont_model_impute = \
        SimpleImputer(strategy='median').fit(dataset[["may_solrad", "june_solrad", "july_solrad", "august_solrad"]])
    # Save the model
    model_name = "6b7fe8c5-5a96-4446-b4e5-c37eb778ab9f"
    fh = open(model_name, "wb")
    pickle.dump(cont_model_impute, fh)
    fh.close()

    cont_features = ["may_solrad", "june_solrad", "july_solrad", "august_solrad"]
    dataset[cont_features] = \
        cont_model_impute.transform(dataset[cont_features])

    # One hot encode categorical values
    encode_features = ["pesticide_material_applied", "fertilizer_material_applied"]
    one_hot_encode_model = \
        OneHotEncoder(handle_unknown='ignore', sparse=False).fit(dataset[encode_features])
    # Save the model
    model_name = "49ba334f-125b-43d6-944d-76f261a85ba8"
    fh = open(model_name, "wb")
...
def get_model_id():
    return "49ba334f-125b-43d6-944d-76f261a85ba8"
...
gbr = GradientBoostingRegressor(n_estimators=420, max_depth=5, learning_rate=0.007, min_samples_split=3)
gbr.fit(train.iloc[:,1:], train.iloc[:,0])
...
for epochs in range (40,400,+20):
  for batch_size in range (4,80,+4):
