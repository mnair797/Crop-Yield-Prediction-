...
def launch_fe(data):
    ...
    columns_list = ["heat_index_3", "heat_index_4", "apparentTemperatureMax", "apparentTemperatureMin", "cloudCover", "dewPoint", "humidity", "precipIntensity", "precipIntensityMax", "precipProbability", "precipTypeIsRain", "pressure", "temperatureMax", "temperatureMin", "visibility", "windBearing", "windSpeed", "NDVI", "Yield", "heat_index_0", "heat_index_1", "heat_index_2"]
    ...
    cont_model_impute = \
        SimpleImputer(strategy='median').fit(dataset[["pressure"]])
    # Save the model
    model_name = "2580c17a-b492-407e-b563-487accabb580"
    fh = open(model_name, "wb")
    pickle.dump(cont_model_impute, fh)
    fh.close()

    cont_features = ["pressure"]
